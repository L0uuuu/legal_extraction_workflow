# Journal officiel des annonces du Tribunal Foncier (Arabe)
# Downloads all issues for a range of years using the year selector.
# Files are saved under:
#   <base_dir>/<year>/
#
# Checkpoint is saved in:
#   checkpoints/download_journal_officiel_tribunal_foncier.checkpoint.json
...
SCRIPT_NAME = "download_journal_officiel_tribunal_foncier.py"


def build_parser():
    return build_common_parser(
        description="Download JORT Tribunal Foncier (AR) PDFs.",
...
def go_to_search(page, cfg):
    ...
    # Step 1: Click "Journal Tribunal Foncier" (M9)
    ...
    print("  ✅ Clicked 'Journal Tribunal Foncier' link")
    page.wait_for_timeout(cfg.page_wait_ms)

    # Step 2: Click "Recherche" (A5)
