...
def navigate_to_table(page, year):
    ...
    try:
        page.wait_for_selector('a[name="M9"]', timeout=10000)
        page.click('a[name="M9"]')
        page.wait_for_load_state("networkidle", timeout=15000)
        print("  ✅ Clicked 'Journal des avis du tribunal immobilier' link")
        page.wait_for_timeout(1000)
    except Exception as e:
        print(f"  ❌ Could not click Journal des avis du tribunal immobilier link: {e}")
        raise
    
    # Step 3: Click "Recherche" (A5)
    try:
        page.wait_for_selector('a[name="A5"]', timeout=10000)
    ...
    try:
        page.wait_for_selector('a[name="A18"]', timeout=10000)
    ...
    # Step 5: Select the year from dropdown (A7)
    try:
...
def get_rows(page):
    ...
    try:
        ...
        page.wait_for_selector('tr[id^="A8_"]', timeout=10000)
