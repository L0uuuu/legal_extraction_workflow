# Journal officiel des lois, décrets, décisions et avis (Arabe)
# Downloads all issues for a range of years using the year selector.
# Files are saved under:
#   <base_dir>/<year>/
#
# Checkpoint is saved in:
#   checkpoints/download_journal_officiel_lois_decrets_decisions_avis.checkpoint.json
...
DEFAULT_BASE_DIR = "pdfs/Journal_Officiel_Lois_Decrets_Decisions_Avis"
SCRIPT_NAME = "download_journal_officiel_lois_decrets_decisions_avis.py"


def build_parser():
    return build_common_parser(
        description="Download JORT Lois/Décrets/Décisions/Avis (AR) PDFs.",
...
def parse_issue(text):
    """
    Parse issue number and date from Arabic text like:
    "156 بتاريخ 31/12/2025"
    """
...
def go_to_search(page, cfg):
    ...
    # Step 1: Click "Recherche journal" (A8)
    page.wait_for_selector('a[name="A8"]', timeout=cfg.selector_timeout_ms)
