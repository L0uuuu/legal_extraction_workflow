...
- **System prompt is on by default (`USE_SYSTEM_PROMPT = True`).** The code-side E-Tafakna system prompt overrides the modelfile's SYSTEM via the Ollama chat API. Set `USE_SYSTEM_PROMPT = False` or pass `--no-system-prompt` to fall back to the modelfile's baked-in SYSTEM instead.
...
- **bge models run on CPU (`device="cpu"`).** Both `bge-m3` and `bge-reranker-v2-m3` are pinned to CPU RAM to leave all GPU VRAM free for `etafakna`. On a 4GB GPU this is required — loading both on GPU alongside the LLM causes OOM.
- **Memory management differs between modes.** In single-shot mode, bge models are freed from RAM (set to `None` + `gc.collect()`) and the LLM is unloaded from Ollama (`keep_alive=0`) after each query. In `--interactive` mode both stay loaded (`keep_alive=300s`) for instant subsequent queries.
- **Both bge models loaded eagerly in interactive mode.** `_get_model()` and `_get_reranker()` are called at startup before the question loop so the first question has no load delay.
- **Reranking fetches a wider candidate pool.** Without `--no-rerank`, Qdrant returns 10 candidates (instead of top_k=3 directly). The cross-encoder then scores each `(question, article_text)` pair jointly — seeing both texts at once — and the top 3 by reranker score are kept. Scores are normalized to [0,1] via sigmoid (`normalize=True`).
- **Empty context when no relevant articles found.** If the best reranker score is below `--confidence-threshold`, articles are set to empty and the model receives `"Aucun article pertinent trouvé."` under the `Documents pertinents:` label, letting the fine-tuned model handle the fallback gracefully.
